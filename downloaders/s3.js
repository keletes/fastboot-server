...
class S3Downloader {
	...
	download() {
		if (!this.configBucket || !this.configKey) {
			this.ui.writeError(
				'no S3 bucket or key provided; not downloading app'
			);
	...
	downloadAppZip() {
		return new Promise((res, rej) => {
			...
			this.ui.writeLine(
				'saving S3 object ' + bucket + '/' + key + ' to ' + zipPath
			);
	...
	unzipApp() {
		...
		return this.exec('unzip -o ' + zipPath).then(() => {
	...
	installNPMDependencies() {
		return this.exec(`cd ${this.outputPath} && npm install`)
			...
			.catch(() =>
				this.ui.writeError('unable to install npm dependencies')
			);
	}
